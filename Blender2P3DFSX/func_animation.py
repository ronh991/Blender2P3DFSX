...
class FSXAssignAnim(bpy.types.Operator):
    """Assign selected animation type to object"""
    bl_label = "Assign Animation"
    bl_idname = "fsx.anim_assign"
    bl_options = {'UNDO'}

    def execute(self, context):
        self.objects = bpy.context.selected_objects
        self.anim = bpy.context.scene.fsx_anims[bpy.context.scene.active_fsx_anim]
        active = context.active_object
        if active.type == 'ARMATURE':
            #  Removed this section, which assigns the same animation to all bones. ON
            #            for Bone in [Bone for Bone in active.data.bones if Bone.select]:
            #                Bone.fsx_anim_tag = self.anim.name
            #                if self.anim.length != '0':
            #                    Bone.fsx_anim_length = self.anim.length #"0-{}".format(self.anim.length)
            #  Replaced it with this section, which will only assign the animation to the active bone. ON
            context.active_bone.fsx_anim_tag = self.anim.name
            if self.anim.length != '0':
                context.active_bone.fsx_anim_length = self.anim.length
        else:
            for ob in self.objects:
                ob.fsx_anim_tag = self.anim.name
                if self.anim.length != '0':
                    ob.fsx_anim_length = self.anim.length
        return {'FINISHED'}


class FSXClearAnim(bpy.types.Operator):
    """Remove animation tag from object"""
    bl_label = "Remove Animation"
    bl_idname = "fsx.anim_clear"
    bl_options = {'UNDO'}

    def execute(self, context):
        if context.active_object.type == 'ARMATURE':
            context.active_bone.fsx_anim_tag = ""
            context.active_bone.fsx_anim_length = "0"
        else:
            context.active_object.fsx_anim_tag = ""
            context.active_object.fsx_anim_length = "0"
