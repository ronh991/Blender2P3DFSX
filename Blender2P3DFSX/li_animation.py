...
class FSXAnimation(bpy.types.PropertyGroup):
    name: bpy.props.StringProperty(name="Item", default="tt")
    length: bpy.props.StringProperty(name="Length", default="0")


class AnimListItem(bpy.types.UIList):
    bl_idname = "ANIM_UL_listItem"

    def draw_item(self, context, layout, data, item, icon, active_data, active_propname):
        layout.label(text=item.name)


bpy.utils.register_class(FSXAnimation)
bpy.utils.register_class(AnimListItem)
