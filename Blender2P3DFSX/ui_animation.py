...
class PFT_Anim(bpy.types.Panel):
    bl_idname = "PFT_PT_Anim"
    ...
    def draw(self, context):
        layout = self.layout
        row = layout.row()
        row.label(text="Assign animation type:", icon='ANIM')
        layout.template_list('ANIM_UL_listItem', "", context.scene, 'fsx_anims', context.scene, 'active_fsx_anim')
        # TODO this doesn't work. Should be saved as a pre-section and loaded afterwards.
        # layout.prop(context.active_object,"fsx_anim_length", text="length")

        row = layout.row()
        row.operator('fsx.anim_assign', icon='ADD')
